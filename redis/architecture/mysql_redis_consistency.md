...
1. 使用延迟双删，先进行缓存清除，再执行update，最后（延迟N秒）再执行缓存清除。
2. 设置缓存时加上过期时间，减少脏数据影响范围
3. 重试机制：当删除缓存失败，加入到消息队列，通过消费者执行，消费者可以从消息队列中重新读取数据，然后再次删除缓存，超过失败次数，报告错误信息
4. 订阅 MySQL binlog，再操作缓存。
